...
def test_search_by_short_title(zotero_db):
    # Test searching by shortTitle with $.shortTitle format
    results = zotero_db.search_json_by_jsonpath("$.shortTitle=TP1")
    assert len(results) == 1
    assert results[0]["key"] == "item1"

    # Test searching by shortTitle with shortTitle format (no $.)
...
def test_invalid_jsonpath(zotero_db):
    # Test with invalid jsonpath expression
    results = zotero_db.search_json_by_jsonpath("invalid_expression")
    assert len(results) == 0


def test_search_by_creator_lastname(zotero_db):
    # Test searching by creator's lastName with $.creators[*].lastName format
