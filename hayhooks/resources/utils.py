...
def read_resource_file(relative_path: str) -> str:
    ...
    try:
        ...
        logger.error(f"Could not find resource file at relative path '{relative_path}' using package 'resources'. Full path attempted: {package_resources}/{relative_path}", exc_info=True)
        raise RuntimeError(
            f"Could not find resource file '{relative_path}' within the 'resources' package."
        ) from e
    except Exception as e:
        logger.error(f"An error occurred while reading resource file '{relative_path}' using package 'resources'. Full path attempted: {package_resources}/{relative_path}", exc_info=True)
        raise RuntimeError(
